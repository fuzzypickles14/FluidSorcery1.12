package com.fuzzpickles14.fluidsorcery.lib;

/**
 * Created by Andrew Toomey on 7/11/2017.
 */
public class LibModDetails {

    public static final String MODID = "fluidsorcery";
    public static final String MOD_NAME = "Fluid Sorcery";
    public static final String VERSION = "0.0.1";

    public static final String CLIENT_PROXY = "com.fuzzypickles14.fluidsorcery.core.proxy.ClientProxy";
    public static final String COMMON_PROXY = "com.fuzzypickles14.fluidsorcery.core.proxy.CommonProxy";
}
